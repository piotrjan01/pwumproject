...
/**
 * A small modification of simple image class given with Simbad framework
 * @author piotrrr
 *
 */
class SimpleImage extends Frame {
	...
	/*****************************
	 * MODIFICATIONS BY PIOTR GWIZDALA
	 * convenience methods
	 **********************************/
	
	public boolean isWhite(int x, int y) {
		if (x<0 || x>=getWidth() || y<0 || y>=getHeight()) return false;
		return getRawPixel(x, y) == Pixels.WHITE; 
	}
	
	public void setWhite(float x, float y) {
		setRawPixel((int)x, (int)y, Pixels.WHITE); 
	}
	
	public boolean isBlack(float x, float y) {
		if (x<0 || x>=getWidth() || y<0 || y>=getHeight()) return false;
		return getRawPixel((int)x, (int)y) == Pixels.BLACK; 
	}
	
	public void setBlack(int x, int y) {
		setRawPixel(x, y, Pixels.BLACK); 
	}

	public boolean isGreen(int x, int y) {
		return getRawPixel((int)x, (int)y) == Pixels.GREEN; 
	}
